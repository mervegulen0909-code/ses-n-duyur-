...
export async function generateMetadata({
  ...
  const { data: perf } = await supabase
    ...
    .eq('id', id)
    .maybeSingle();
  // The root loading boundary streams a 200 shell before metadata resolves,
  // so a status-changing notFound() is impossible here; keep crawlers out of
  // missing/hidden performances with noindex and an honest title instead of
  // the "Performance — — on VoxScore" placeholder.
  if (!perf) {
    return {
      title: 'Performance not found — VoxScore',
